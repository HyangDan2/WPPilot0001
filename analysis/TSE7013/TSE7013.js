const labels = {
    ko: {
        heroTitle: "IHI Corporation 글로벌 방산/항공우주 분석",
        heroCopy: "일본 IHI를 중심으로 미쓰비시중공업, 가와사키중공업, 록히드 마틴, 한화에어로스페이스, 라인메탈의 흐름을 함께 보는 정적 분석 페이지입니다.",
        dataStatus: "데이터 기준",
        latestClose: "최신 종가",
        marketCap: "시가총액",
        fyRevenue: "FY2025 매출",
    ...
    en: {
        ...
        heroCopy: "A static analysis dashboard for IHI, Mitsubishi Heavy, Kawasaki Heavy, Lockheed Martin, Hanwha Aerospace and Rheinmetall.",
        dataStatus: "Data As Of",
        latestClose: "Latest Close",
        marketCap: "Market Cap",
        fyRevenue: "FY2025 Revenue",
        ...
        viewStateValue: "Short rebound inside medium correction",
        revenue: "Revenue",
    ...
    ja: {
        ...
        heroCopy: "IHIを中心に、三菱重工業、川崎重工業、Lockheed Martin、Hanwha Aerospace、Rheinmetallを比較する静的分析ページです。",
        dataStatus: "データ基準",
        latestClose: "最新終値",
        marketCap: "時価総額",
        fyRevenue: "FY2025売上高",
...
function renderSummary() {
    ...
    document.getElementById("marketCap").textContent = targetPeer.market_cap;
    document.getElementById("valuationLine").textContent = `PER ${targetPeer.pe} / RSI ${targetPeer.rsi}`;
    document.getElementById("fyRevenue").textContent = jpyB(financials.revenue);
    document.getElementById("fyProfit").textContent = `${labels[activeLang].opProfit} ${jpyB(financials.operating_profit)} (${financials.operating_margin_percent}%)`;
...
function renderFinancials() {
    ...
    const rows = [
        [labels[activeLang].revenue, `${jpyB(f.revenue)} / YoY ${f.revenue_yoy_percent}%`],
        [labels[activeLang].opProfit, `${jpyB(f.operating_profit)} / YoY ${f.operating_profit_yoy_percent}%`],
